...
public class ConsoleApp
{
    ...
    static void WriteInputOptions(CommonActions options)
    {
        ...
        if (options.HasFlag(CommonActions.ReturnLoanedBook)) Console.WriteLine(" - \"r\" to mark as returned");
        if (options.HasFlag(CommonActions.ExtendLoanedBook)) Console.WriteLine(" - \"e\" to extend the book loan");
        if (options.HasFlag(CommonActions.RenewPatronMembership)) Console.WriteLine(" - \"m\" to extend patron's membership");
        if (options.HasFlag(CommonActions.SearchPatrons)) Console.WriteLine(" - \"s\" for new search");
        if (options.HasFlag(CommonActions.SearchBooks)) Console.WriteLine(" - \"b\" to check for book availability");
        if (options.HasFlag(CommonActions.Quit)) Console.WriteLine(" - \"q\" to quit");
        if (options.HasFlag(CommonActions.Select)) Console.WriteLine("Or type a number to select a list item.");
    }
    ...
    async Task<ConsoleState> PatronDetails()
    {
        ...
        else if (action == CommonActions.Quit) return ConsoleState.Quit;
        else if (action == CommonActions.SearchPatrons) return ConsoleState.PatronSearch;
        else if (action == CommonActions.SearchBooks) return await SearchBooks();
    ...
    async Task<ConsoleState> SearchBooks()
    {
        ...
        // LINQ Query 1: Find matching book by title (case-insensitive partial match)
        ...
        // LINQ Query 2: Find all book items (physical copies) for this book
        var bookItems = _jsonData.BookItems?
            .Where(bi => bi.BookId == book.Id)
            .ToList();

        if (bookItems == null || !bookItems.Any())
        {
            Console.WriteLine($"{book.Title} has no physical copies in the library.");
            return ConsoleState.PatronDetails;
        }

        // LINQ Query 3: Check for active loans on any copy of this book
        var activeLoan = _jsonData.Loans?
            .Where(l => l.ReturnDate == null) // Only unreturned loans
            .FirstOrDefault(l => bookItems.Any(bi => bi.Id == l.BookItemId));

        // Evaluate availability status
        if (activeLoan == null)
        {
